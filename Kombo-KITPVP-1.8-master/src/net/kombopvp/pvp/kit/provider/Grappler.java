...
public class Grappler extends KitHandler{
	...
	public void usarbbf(PlayerMoveEvent e) {
		...
		if (Jump.recebeu.containsKey(p.getName()) && EnderMageReal.isSpawn(p.getLocation())
				&& e.getPlayer().getLocation().getY() > KomboPvP2.getInstance().getConfig().getInt("SpawnAltura")
	...
	public void usarf(PlayerMoveEvent e) {
		...
		if ((p.getItemInHand().getType().equals(Material.LEASH))) {
			if (KitManager.getPlayer(e.getPlayer().getName()).hasKit(this)) {
				Player target = p;
				if ((p.getLocation().getY() > KomboPvP2.getInstance().getConfig().getInt("SpawnAltura") - 12)
...
public void onPlayerInteract(PlayerInteractEvent event) {
	...
	Player target = p;
	if ((p.getLocation().getY() > KomboPvP2.getInstance().getConfig().getInt("SpawnAltura") - 12) && EnderMageReal.isSpawn(p.getLocation()) && !GladiatorListener2.combateGlad.containsKey(target) && !net.kombopvp.kit2.GladiatorListener.combateGlad.containsKey(target)) {
...
public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
    ...
    if (event.getEntity() instanceof Player && event.getDamager() instanceof Player) {
        ...
        if (KitManager.getPlayer(player.getName()).hasKit(this)) {
        	addCooldown(player , KomboPvP2.getInstance().getConfig().getInt("GrapplerCooldown"));
        }
    }
}
