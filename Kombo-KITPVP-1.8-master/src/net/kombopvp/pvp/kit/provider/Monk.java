...
public class Monk extends KitHandler {
	...
	public void onEntityDamageByEntity(PlayerInteractAtEntityEvent event) {
		...
		else if (event.getPlayer().getLocation().getY() > KomboPvP2.getInstance().getConfig().getInt("SpawnAltura") && KitManager.getPlayer(event.getPlayer().getName()).hasKit(this)  && EnderMageReal.isSpawn(player.getLocation())) {
