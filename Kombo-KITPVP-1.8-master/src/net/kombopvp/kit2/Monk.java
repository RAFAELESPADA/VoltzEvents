...
public class Monk extends KitHandler2 {
	...
	public void onEntityDamageByEntity(PlayerInteractAtEntityEvent event) {
		...
		else if (event.getPlayer().getLocation().getY() > KomboPvP2.getInstance().getConfig().getInt("SpawnAltura") && KitManager2.getPlayer(event.getPlayer().getName()).haskit2(this)  && EnderMageReal.isSpawn(player.getLocation())) {
		...
		if (!KitManager2.getPlayer(player.getName()).haskit2(this) 
           ...
           Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)KomboPvP2.getInstance(), (Runnable)new Runnable() {
               @Override
               public void run() {
