...
public class NinjaWarp extends Warp
{
    private boolean cocoaInv;
    
    public NinjaWarp() {
        this.setName("Ninja");
        this.setDefaultKit(Main.getInstance().getKitManager().searchKit("Ninja"));
        this.setPvp(true);
        this.setSpawn(new Location((World)Bukkit.getWorlds().get(0), Main.getInstance().getConfig().getInt("NinjaX"), Main.getInstance().getConfig().getInt("NinjaY"), Main.getInstance().getConfig().getInt("NinjaZ")));
    }
