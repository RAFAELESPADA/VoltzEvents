...
public class GamerRepository
{
    ...
    public void insert(final Gamer occurrence) {
        try (final Connection connection = this.dataSource.getConnection()) {
            final PreparedStatement statement = connection.prepareStatement("INSERT INTO `players`(`name`, `tags`, `group`, `punishments`) VALUES ('" + occurrence.getName() + "', '" + occurrence.getTagsString() + "', '" + occurrence.getGroup().getName() + "', '" + JsonUtils.makePunishments(occurrence) + "');");
    ...
    public void update(final Gamer occurrence) {
        try (final Connection connection = this.dataSource.getConnection()) {
            if (this.contains(occurrence.getName())) {
                final PreparedStatement statement = connection.prepareStatement("UPDATE `players` SET `tags` = '" + occurrence.getTagsString() + "', `group` = '" + occurrence.getGroup() + "', `punishments` = '" + JsonUtils.makePunishments(occurrence) + "' WHERE `name` = '" + occurrence.getName() + "';");
                statement.executeUpdate();
                statement.close();
            }
            else {
