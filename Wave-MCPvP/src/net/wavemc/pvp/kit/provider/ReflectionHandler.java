...
public final class ReflectionHandler {
  ...
  public static Constructor<?> getConstructor(Class<?> clazz, Class[] parameterTypes) {
    ...
    int j = (arrayOfConstructor = (Constructor[])clazz.getConstructors()).length;
    for (int i = 0; i < j; i++) {
  ...
  public static Method getMethod(Class<?> clazz, String name, Class[] parameterTypes) {
    ...
    for (int i = 0; i < j; i++) {
      ...
      if (m.getName().equals(name) && 
        DataType.equalsArray(DataType.convertToPrimitive(m.getParameterTypes()), p))
        return m; 
    } 
    return null;
  }
  ...
  public static void setValues(Object instance, FieldPair[] pairs) throws Exception {
    FieldPair[] arrayOfFieldPair;
    int j = (arrayOfFieldPair = pairs).length;
    for (int i = 0; i < j; i++) {
      FieldPair pair = arrayOfFieldPair[i];
  ...
  public enum PacketType {
    ...
    PLAY_IN_CHAT(
      "PacketPlayInChat"),
    PLAY_IN_CLIENT_COMMAND(
      "PacketPlayInClientCommand"),
    PLAY_IN_CLOSE_WINDOW(
      "PacketPlayInCloseWindow"),
    PLAY_IN_CUSTOM_PAYLOAD(
      "PacketPlayInCustomPayload"),
    PLAY_IN_ENCHANT_ITEM(
      "PacketPlayInEnchantItem"),
    PLAY_IN_ENTITY_ACTION(
      "PacketPlayInEntityAction"),
    PLAY_IN_FLYING(
      "PacketPlayInFlying"),
    PLAY_IN_HELD_ITEM_SLOT(
    ...
    PLAY_OUT_MAP_CHUNK_BULK(
      "PacketPlayOutMapChunkBulk"),
    PLAY_OUT_MULTI_BLOCK_CHANGE(
      "PacketPlayOutMultiBlockChange"),
    PLAY_OUT_NAMED_ENTITY_SPAWN(
      "PacketPlayOutNamedEntitySpawn"),
    PLAY_OUT_NAMED_SOUND_EFFECT(
      "PacketPlayOutNamedSoundEffect"),
    PLAY_OUT_OPEN_SIGN_EDITOR(
      "PacketPlayOutOpenSignEditor"),
    PLAY_OUT_OPEN_WINDOW(
      "PacketPlayOutOpenWindow"),
    PLAY_OUT_PLAYER_INFO(
      "PacketPlayOutPlayerInfo"),
    PLAY_OUT_POSITION(
      "PacketPlayOutPosition"),
    PLAY_OUT_REL_ENTITY_MOVE(
      "PacketPlayOutRelEntityMove"),
    PLAY_OUT_REL_ENTITY_MOVE_LOOK(
      "PacketPlayOutRelEntityMoveLook"),
    PLAY_OUT_REMOVE_ENTITY_EFFECT(
      "PacketPlayOutRemoveEntityEffect"),
    PLAY_OUT_RESPAWN(
      "PacketPlayOutRespawn"),
    PLAY_OUT_SCOREBOARD_DISPLAY_OBJECTIVE(
      "PacketPlayOutScoreboardDisplayObjective"),
    PLAY_OUT_SCOREBOARD_OBJECTIVE(
      "PacketPlayOutScoreboardObjective"),
    PLAY_OUT_SCOREBOARD_SCORE(
      "PacketPlayOutScoreboardScore"),
    PLAY_OUT_SCOREBOARD_TEAM(
      "PacketPlayOutScoreboardTeam"),
    PLAY_OUT_SET_SLOT(
      "PacketPlayOutSetSlot"),
    PLAY_OUT_SPAWN_ENTITY(
      "PacketPlayOutSpawnEntity"),
    PLAY_OUT_SPAWN_ENTITY_EXPERIENCE_ORB(
  ...
  public enum SubPackageType {
    ...
    SubPackageType() {
      this.name = ReflectionHandler.PackageType.CRAFTBUKKIT + "." + name().toLowerCase();
    }
